...
app.get('/api/UserWorkouts',(req, res, next) => {
  const id = getID(req)
  const sql = "select workout_events.id, workout_events.date, workout_events.workout_id, workout_events.is_done from training_plans Inner join workout_events on training_plans.id= workout_events.training_plan_id where training_plans.client_id = ?"
  const params = [id]
  db.all(sql, params, (err, rows) => {
    if (err) {
      res.status(400).json({ error: err.message })
...
app.get('/api/GetTrainingplanIdByClientID/:id',(req, res, next) => {
  const sql = "select id from training_plans where training_plans.client_id =?"
  const params = [req.params.id]
  db.all(sql, params, (err, rows) => {
    if (err) {
      res.status(400).json({ error: err.message })
